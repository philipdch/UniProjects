...
def handle(req):
    ...
    try:
        ...
        if num_objects == TOTAL_PARTS:
            reduce_output = {"50s":0, "60s":0, "70s":0, "80s":0, "90s":0, "00s":0, "10s":0 }
            ...
            result = client.put_object(
                "reducer3", "final_results.json", io.BytesIO(str.encode(final_results)), length=-1, part_size=10 * 1024 * 1024,
            )
            print(
...
# Calculates the average trip duration for each key (age group)
def reducer(chunk, output_dict):

    data = csv.reader(io.StringIO(chunk), delimiter=",", quotechar='"')
    count_dict = {"50s":0, "60s":0, "70s":0, "80s":0, "90s":0, "00s":0, "10s":0 }
    for row in data:
        key = row[0]
        value = int(row[1])
    ...
    for key in output_dict.keys():
        if(count_dict[key] == 0):
            ...
            output_dict[key] = output_dict[key] / count_dict[key]
    return output_dict
