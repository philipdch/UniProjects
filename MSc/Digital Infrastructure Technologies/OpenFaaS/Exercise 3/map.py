...
def handle(req):
    ...
    try:
        ...
        result = client.put_object(
            "mapper3", filename, io.BytesIO(str.encode(intermediate_results)), length=-1, part_size=10 * 1024 * 1024,
        )
        print(
...
# produces intermediate results. Given each subscribers birth year, 
# maps each one of them to an age group with the respective duration of
# their trip (in seconds)
def mapper(chunk):
    ...
    print(header)
    for row in data:
        trip_duration = int(row[0])
        customer_type = row[12]
        if(customer_type == "Subscriber"):
            year = int(row[13])
            if year <= 1959:
                map_output.append(["50s",trip_duration])
            elif year <= 1969:
                map_output.append(["60s",trip_duration])
            elif year <= 1979:
                map_output.append(["70s",trip_duration])
            elif year <= 1989:
                map_output.append(["80s",trip_duration])
            elif year <= 1999:
                map_output.append(["90s",trip_duration])
            elif year <= 2009:
                map_output.append(["00s",trip_duration])
            else:
                map_output.append(["10s",trip_duration])
    return map_output
